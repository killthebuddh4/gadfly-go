...
func Http(trajectory *traj.Trajectory, eval eval.Eval) (value.Value, error) {
	...
	requestHeaders, ok := requestHeadersV.(map[string]value.Value)
	...
	requestBody, ok := requestBodyV.(map[string]value.Value)
	...
	value := make(map[string]value.Value)
